...
import {
  ...
  setSparklineStreamHealthy,
} from './sparklineHistoryHub';

export { useSparklineHistory, useSparklineRevision, useSparklineBaselineReady } from './sparklineHistoryHub';
export { prefetchSparklineBaselines, ensureSparklineBaseline } from './sparklineBaselineLoader';
export { isSparklineStreamHealthy } from './sparklineHistoryHub';
...
function applySnapshot(prices: Record<string, LivePriceQuote>) {
  ...
  for (const [symbol, quote] of Object.entries(upper)) {
    appendSparklineLivePrice(symbol, quote.price);
...
function applyUpdates(updates: { symbol: string; price: number; percentChange24h: number }[]) {
  ...
  for (const u of updates) {
    ...
    if (changed) {
      ...
      appendSparklineLivePrice(symbol, u.price);
      bumpSymbolVersion(symbol);
    }
  }
  hubQuotes = next;
...
function syncConnection() {
  ...
  socket.onopen = () => {
    ...
    setSparklineStreamHealthy(true);
    bumpConnectionVersion();
    sendSubscribe();
  };
  ...
  socket.onclose = () => {
    if (mySession !== socketSession) return;
    ws = null;
    setSparklineStreamHealthy(false);
    bumpConnectionVersion();
