...
/**
 * Full-screen overlay: dim + blur, centered glass card animates from measured
 * focus origin (scale 1 → 1.05, opacity ramp).
 *
...
>(function FloatingFieldFocusLayer(
  ...
  const offsets = useMemo(() => {
    ...
    const targetCenterY =
      (windowHeight - insets.top - insets.bottom) / 2 + insets.top;
    return {
      x: sourceCenter.cx - windowWidth / 2,
      y: sourceCenter.cy - targetCenterY,
    };
  }, [sourceCenter, windowWidth, windowHeight, insets.top, insets.bottom]);
  ...
  }, [visible, activeField, sourceKey, progress]);
