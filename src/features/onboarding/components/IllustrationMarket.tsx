...
function IllustrationMarketInner({ width, height }: IllustrationMarketProps) {
  ...
  const { stroke, muted, primary, neutral } = useMemo(() => buildIllustrationMarketPalette(tokens), [tokens]);
  ...
  return (
    <View style={{ width: w, height: h }}>
      <Svg width={w} height={h} viewBox={`0 0 ${w} ${h}`}>
        <Rect x={pad} y={h * 0.08} width={w - pad * 2} height={h * 0.14} rx={8} fill={neutral[100]} />
        ...
        <Rect x={w * 0.62} y={h * 0.08} width={w * 0.22} height={h * 0.065} rx={6} fill={neutral[200]} />
...
function buildIllustrationMarketPalette(tokens: ThemeTokens) {
  const c = tokens.colors;
  return {
    stroke: c.success[600],
    muted: c.neutral[400],
